...
export default function Renovasi({ userRole, renovations = [] }) {
  ...
  const handleSave = (e) => {
    ...
    const payload = {
      ...
      pajak_pph: Number(formData.pajak_pph) || 0,
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "Dalam Proses": return "bg-orange-50 text-orange-700 border-orange-200";
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-5 overflow-y-auto flex-1 custom-scrollbar gap-6 flex flex-col">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1">Status Proyek *</label>
                    <select
                      value={formData.status}
                      onChange={(e) => setFormData((p) => ({ ...p, status: e.target.value }))}
                      disabled={isSaving}
                      className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-xs"
                    >
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      disabled={isSaving}
                      className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-xs custom-scrollbar"
